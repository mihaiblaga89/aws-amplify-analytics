 ...
 */
import { ConsoleLogger as Logger, ClientDevice, Platform, Credentials, Signer } from '@aws-amplify/core';
...
export default class AWSPinpointProvider implements AnalyticsProvider {
	...
	private async _updateEndpoint(params): Promise<boolean> {
		...
		const request = await this._endpointRequest(config, event);
		const update_params = {
			ApplicationId: appId,
	...
	private _endpointRequest(config, event) {
		...
		const { Address, RequestId, Attributes, UserAttributes, UserId, OptOut } = event;
